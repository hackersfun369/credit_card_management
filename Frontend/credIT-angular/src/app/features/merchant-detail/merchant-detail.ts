...
export class MerchantDetail implements OnInit {
  get customerMode() { return this.router.url.startsWith('/my-account/'); }
  ...
  cardPath(card: any) { return (this.customerMode ? '/my-account/cards/' : '/cards/') + this.cardId(card); }
  transactionPath(transaction: any) { return (this.customerMode ? '/my-account/transactions/' : '/transactions/') + this.transactionId(transaction); }
  ...
  async load() { this.loading = true; this.error = ''; try { const merchantId = this.route.snapshot.paramMap.get('id'); let merchants: any; let transactions: any; let cards: any; if (this.customerMode) { const [dashboard, availableMerchants] = await Promise.all([this.api.request<any>('auth', '/api/customer-portal/dashboard'), this.api.request<any>('auth', '/api/customer-portal/merchants')]); merchants = availableMerchants; transactions = dashboard?.transactions || []; cards = dashboard?.cards || []; const associated = this.rows(transactions).some((item: any) => String(item?.merchantId) === String(merchantId)); if (!associated) { this.error = 'This merchant is not associated with your transactions.'; return; } } else { [merchants, transactions, cards] = await Promise.all([this.api.request<any>('merchants', '/merchants'), this.api.request<any>('transactions', '/transactions'), this.api.request<any>('cards', '/card')]); } this.merchant = this.rows(merchants).find((item: any) => String(this.id(item)) === String(merchantId)); this.transactions = this.rows(transactions).filter((item: any) => String(item?.merchantId) === String(merchantId)); const usedCardNumbers = new Set(this.transactions.map(item => this.digits(item?.cardNumber)).filter(Boolean)); this.cards = this.rows(cards).filter((item: any) => usedCardNumbers.has(this.digits(item?.cardNumber))); if (!this.merchant) this.error = 'Merchant not found.'; } catch (error: any) { this.error = error?.message || 'Unable to load merchant details.'; } finally { this.loading = false; this.cdr.detectChanges(); } }
}