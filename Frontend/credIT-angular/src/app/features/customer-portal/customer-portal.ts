...
export class CustomerPortal implements OnInit {
  ...
  get pendingCardRequests(){return (this.data.cardRequests||[]).filter((request:any)=>String(request?.status||'PENDING').toUpperCase()==='PENDING').length;}
  ...
  hasCardType(type:string){const expected=type.toUpperCase();return this.cards.some(card=>String(card.cardType||'').toUpperCase()===expected)||(this.data.cardRequests||[]).some((request:any)=>String(request?.status||'PENDING').toUpperCase()==='PENDING'&&String(request?.cardType||'').toUpperCase()===expected);}
